...
description: Create a concise, risk-prioritized QA test plan or lightweight implementation test strategy from product context, application exploration, documentation, and user goals. Use when a feature or project needs a test strategy before concrete cases are written, or when planning must decide how a change will be tested and independently validated.
---

# Generate Test Plan

Follow the mode that matches the request. The skill body is self-contained: use the user's request, repository files, runtime artifacts, and configured capabilities/providers available in the current session.

## Modes

### Lightweight planning mode

Use this mode when implementation planning needs a compact test strategy
decision rather than a full QA test-plan artifact. Decide how the change earns
confidence and which independent validation workflow should run after
implementation. Do not create test cases, run tests, or write test-plan files in
this mode.
...
- Primary confidence source: <unit/integration/e2e/QA/manual>
- Acceptance coverage:
  - `<AC1>` — <test type + path/area/workflow>
  - `<AC2>` — <test type + path/area/workflow>
- Required developer tests:
  - <test file or description>
- Required QA validation:
  - Workflow: <run existing automation / add automation / generic verification / ticket verification / deployment verification>
  - Scope: <feature/flow/criteria to validate>
  - Environment: <local/preview/staging/etc.>
- Manual checks:
  - <steps or none>
- Not testing in this task:
  - <non-goal and reason>
```

Use these heuristics:

- Bug: include a regression test or explain why it cannot be automated.
- Feature: include tests for the happy path and highest-risk edge case.
- Refactor: require characterization tests before behavior-preserving changes.
- Auth, billing, permissions, migrations, and async workers: prefer integration
  tests plus black-box validation.
- Pure docs/process tasks: manual validation notes are usually enough.

### Full QA test-plan mode

Use the full workflow below when the user asks to generate a QA test plan,
explore product behavior for test planning, or produce a durable test-plan
artifact.
...
Lightweight planning mode has no required external capability.
...
Read template from `.sdlc/runtime/templates/test-plan-template.md` and fill it in. The FIRST LINE must be `<!-- Generated by AI -->` (EU AI Act Article 50(2)). Replace placeholders (`[PROJECT_NAME]`, `[DATE]`). If API endpoints were discovered, include an "API Endpoints" section. Keep under 200 lines. Save to `test-plan.md` in project root.
