---
name: verify-changes-on-deployment
description: Verify a deployed preview or environment with scoped automated tests, manual checks, and routed reporting. Use when a deployment URL, preview build, Vercel event, or environment-specific release needs QA validation.
---
...
### Step 1: Verify Changes on Deployment — Overview

# Verify Changes on Deployment

Verifies code changes by running tests against a deployed preview environment.

**Trigger sources:** `com.github.deployment_status`, `com.vercel.deployment`, AWS deployment notifications.

**Flow:** Extract deployment context (preview URL, commit, PR) → determine test scope → create tests for gaps → run tests against preview URL → report results as PR comment + check run.

**Explore first, ask later:** The preview URL is a live deployed app — use it as your primary source of truth. Explore the feature in the browser to resolve ambiguity about what changed. Do NOT stop for clarification when you have a working preview URL and a change description. If the PR lookup returns mismatched data, trust the deployment payload's PR title and description over the external lookup.
...
The input contains a deployment event payload with preview URL, commit SHA, environment, PR reference, and repository details.

### Step 4: Extract Deployment Context

Parse the deployment event payload to extract verification context:

**GitHub Deployment Status:** Preview URL from `deployment_status.environment_url` (fallback: `target_url`), commit SHA from `deployment.sha`, branch from `deployment.ref`, PR number from `pull_request.number`, environment from `deployment.environment`, repo from `repository.full_name`.

**Vercel Deployment:** Preview URL from `deployment.url` or `deployment.alias[0]`, commit SHA from `deployment.meta.githubCommitSha`, branch from `deployment.meta.githubCommitRef`, environment from `deployment.target`.

Build CHANGE_CONTEXT with: trigger='DEPLOYMENT', previewUrl, commitSha, branch, prNumber, environment, githubRepo.

**IMPORTANT**: Set `BASE_URL` to the preview URL. All tests run against this deployed preview.

### Step 5: Look Up PR Details (when source control is configured)
...
Use the configured source-control provider to fetch PR metadata (title, description, comments, commit messages) using the repo and PR number from CHANGE_CONTEXT.

**IMPORTANT — Blackbox testing**: Do NOT look at code diffs, changed files, or source code. Test scope is determined from the PR description only, never from code.

Use PR title and description to enrich CHANGE_CONTEXT for test scope determination.
...
### Step 16: Report Results to PR & Check Run (when source control is configured)
...
**Test Coverage Section**: Include in comment — changed areas, existing coverage, coverage gaps, and new tests created to fill gaps.
