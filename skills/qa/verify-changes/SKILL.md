---
name: verify-changes
description: Verify code or product changes with black-box scope analysis, targeted automated tests, manual checklists, and reporting. Use when the user asks for QA verification without a ticket-specific or deployment-specific trigger.
---
...
# Verify Changes — Generic Verification Workflow

Generic fallback for manual/CLI invocations and unstructured triggers. Structured webhook events use specialized tasks:
- **Ticket events** (Jira, Asana) → `verify-changes-on-ticket`
- **Deployment events** (GitHub, Vercel) → `verify-changes-on-deployment`

**Trigger sources handled here:** Manual CLI, Slack messages, GitLab MR events, GitHub PR events, CI/CD pipeline invocations.

**Explore first, ask later:** When the app is accessible, always explore the feature in the browser before asking clarification. Use the change description to guide exploration — resolve ambiguity through discovery, not by asking the team. Only stop for clarification if the feature is genuinely absent and no authoritative trigger claims it exists.
...
The input format varies by source. It may be:
- Natural language description (manual CLI)
- PR/MR URL or number
- Structured webhook payload (GitHub PR, GitLab MR)
- Slack message with verification request

### Step 4: Detect Available Context

Analyze the input to determine trigger type and extract context:

- **GitHub PR payload** (contains `pull_request` object): Extract PR number, title, description, author, branches. Trigger: GITHUB_PR.
- **GitLab MR payload** (contains `object_attributes` or `merge_request`): Extract MR IID, title, description, source/target branches. Trigger: GITLAB_MR. Pass MR IID and project identifier when using source-control provider (`glab api` with Bearer auth).
- **Slack message payload** (contains `event.type: "message"`): Extract message text, channel, thread. Parse for PR numbers, issue IDs, URLs. Trigger: SLACK_MESSAGE.
- **CI/CD environment** (`CI=true` env var): Read GITHUB_REF, GITHUB_SHA, GITHUB_BASE_REF. Trigger: CI_CD.
- **Manual/natural language** (default): Parse for PR URLs, issue IDs, feature descriptions. Trigger: MANUAL.

Build a unified CHANGE_CONTEXT object with: trigger type, title, description, and any available identifiers (prNumber, mrIid, githubRepo, gitlabProject, slackChannel, slackThread).

### Step 5: Look Up PR/MR Details (when source control is configured)
...
Use the configured source-control provider to fetch PR/MR metadata (title, description, comments, related issues, authors). Pass the repo and PR/MR identifier from CHANGE_CONTEXT. For GitLab, pass MR IID — the agent uses `glab api` with `GITLAB_PROJECT_ID`.

**IMPORTANT — Blackbox testing**: Do NOT look at code diffs, changed files, or source code. Test scope is determined from descriptions only, never from code.

Use PR/MR description to enrich CHANGE_CONTEXT for test scope determination.
...
### Step 9: Update Test Plan (Living Document)

If new test cases were created in the previous step, update `test-plan.md`: mark covered features with ✅ and TC-XXX IDs, update `updated_at` in metadata. Skip if test-plan.md does not exist or no new tests were created.

### Step 10: Execute Automated Tests
...
### Step 13: Fix Test Issues Automatically
...
### Step 14: Log Product Bugs (when issue tracker is configured)
...
### Step 15: Generate Manual Verification Checklist
...
### Step 16: Aggregate Verification Results
...
**MANUAL** → Terminal: Format comprehensive markdown report with change summary, test results, manual verification checklist, recommendation, and test artifact paths.

**SLACK_MESSAGE** → Include concise results in output with verification summary, critical failures, manual checklist summary, and next steps. The inbox agent will post the thread reply.

**GITLAB_MR** → MR comment: use the configured source-control provider to post a note with status, test summary, failed tests, product bugs, manual verification, and recommendation. Agent uses `glab api`.

**GITHUB_PR** → PR comment: Post comprehensive comment with status, test results table, failed tests, tests fixed, product bugs, manual verification, and recommendation.

**CI_CD** → Build log + exit code: Output results to CI log. Exit 0 if all passed, exit 1 if tests failed or critical bugs found.

### Step 20: Handle Special Cases
