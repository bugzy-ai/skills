---
name: bugzy-issue-gitlab
description: Manage GitLab Issues issues for QA findings, story updates, duplicate checks, comments, and workflow transitions. Use when GitLab Issues is the configured issue tracker.
---
...
Use when QA findings, product bugs, story QA status, duplicate checks, or verification notes need to be created or updated in GitLab Issues.

## Source basis

Derived from `packages/bugzy/src/subagents/templates/issue-tracker/gitlab.ts` and the issue-tracker metadata entry for gitlab.

## Workflow

1. Use glab for GitLab issues, labels, comments, and issue state.
