---
name: bugzy-issue-jira
description: Manage Jira issues for QA findings, story updates, duplicate checks, comments, and workflow transitions. Use when Jira is the configured issue tracker.
---
...
Use when QA findings, product bugs, story QA status, duplicate checks, or verification notes need to be created or updated in Jira.

## Source basis

Derived from `packages/bugzy/src/subagents/templates/issue-tracker/jira.ts` and the issue-tracker metadata entry for jira.

## Workflow

1. Use jira-cli for Jira Cloud issues, JQL, comments, transitions, fields, and projects.
